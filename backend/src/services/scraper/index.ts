...
export async function scrapeEastmoney(): Promise<NewsItemRaw[]> {
  try {
    const url =
    ...
    const newsList: NewsItemRaw[] = [];
    if (data?.data?.list) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      for (const item of data.data.list as any[]) {
        const newsUrl =
          item.url || `https://finance.eastmoney.com/a/${item.newsID}.html`;
        newsList.push({
    ...
    console.error("[东方财富] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeSina(): Promise<NewsItemRaw[]> {
  try {
    const url =
      "https://feed.mix.sina.com.cn/api/roll/get?pageid=153&lid=2516&k=&num=20&page=1&r=0." +
      Date.now();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data: any = await fetchWithRetry(url);
    const newsList: NewsItemRaw[] = [];
    if (data?.result?.data) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      for (const item of data.result.data as any[]) {
        newsList.push({
          title: (item.title || "").trim(),
    ...
    console.error("[新浪财经] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeTHS(): Promise<NewsItemRaw[]> {
  try {
    const url =
      "https://news.10jqka.com.cn/tapp/news/push?stock=&page=1&limit=20&type=0";
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data: any = await fetchWithRetry(url);
    const newsList: NewsItemRaw[] = [];
    ...
    console.error("[同花顺] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeXueqiu(): Promise<NewsItemRaw[]> {
  try {
    const url =
      "https://xueqiu.com/statuses/hot/listV2.json?since_id=-1&max_id=0&size=20";
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data: any = await fetchWithRetry(url);
    const newsList: NewsItemRaw[] = [];
    if (data?.items) {
      ...
      for (const item of data.items as any[]) {
        ...
        newsList.push({
          ...
          publishedAt: new Date(
            original.created_at ? original.created_at * 1000 : Date.now(),
          ),
        });
      }
    }
    ...
    console.error("[雪球] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrape36Kr(): Promise<NewsItemRaw[]> {
  try {
    ...
    if (data?.data?.items) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      for (const item of data.data.items as any[]) {
        newsList.push({
          title: (item.title || "").trim(),
    ...
    console.error("[36Kr] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeJiemian(): Promise<NewsItemRaw[]> {
  try {
    ...
    console.error("[界面] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeCe36(): Promise<NewsItemRaw[]> {
  try {
    ...
    console.error("[财新] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeEastmoneySearch(
  query: string,
): Promise<NewsItemRaw[]> {
  try {
    ...
    console.error("[东方财富搜索] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeSinaSearch(query: string): Promise<NewsItemRaw[]> {
  try {
    const encodedQuery = encodeURIComponent(query);
    ...
    console.error("[新浪搜索] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeTHSSearch(query: string): Promise<NewsItemRaw[]> {
  try {
    const encodedQuery = encodeURIComponent(query);
    ...
    console.error("[同花顺搜索] 采集失败:", (error as Error).message);
    return [];
  }
}

export async function scrapeXueqiuSearch(
  query: string,
): Promise<NewsItemRaw[]> {
  try {
    const encodedQuery = encodeURIComponent(query);
    ...
    console.error("[雪球搜索] 采集失败:", (error as Error).message);
    return [];
  }
}
