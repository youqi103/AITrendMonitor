...
async function saveNewsItems(items: NewsItemRaw[]): Promise<number> {
  let saved = 0;
...
export async function runKeywordMonitor(io: any) {
  ...
  for (const term of monitorTerms) {
    for (const news of recentNews) {
      ...
      try {
        ...
        if (!relevance.isRelated || relevance.relevanceScore < 0.5) continue;
        ...
        console.error(`[监控] AI 判断失败:`, (error as Error).message);
      }
    }
  }
...
export async function runVerifyNews() {
  ...
  for (const news of unverified) {
    try {
      const result = await verifyAuthenticity(
        news.title,
        news.content,
        news.source,
      );
