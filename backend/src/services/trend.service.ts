...
export async function runTrendDiscovery(io: any) {
  ...
  const newsList = recentNews
    .map(
      (n, i) =>
        `${i + 1}. [${n.source}] ${n.title}: ${n.content.substring(0, 100)}`,
    )
    .join("\n");
  ...
  try {
    ...
    for (const trend of result.trends) {
      ...
      const relatedNews = recentNews.find(
        (n) =>
          n.title.includes(trend.title.substring(0, 4)) ||
    ...
    console.error("[热点] 发现失败:", (error as Error).message);
  }
}
