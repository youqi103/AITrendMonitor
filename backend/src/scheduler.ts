...
export function initScheduler(io: any) {
  ioInstance = io;
  ...
  cron.schedule(fastInterval, async () => {
    ...
    await runScrapeAll();
    if (ioInstance) await runKeywordMonitor(ioInstance);
  });
  ...
  cron.schedule("*/15 * * * *", async () => {
    if (ioInstance) await runVerifyNews();
  });

  // 热点发现
  cron.schedule(trendInterval, async () => {
    if (ioInstance) await runTrendDiscovery(ioInstance);
  });
...
export async function runInitialTasks(io: any) {
