...
import { scrapeEastmoneySearch, scrapeSinaSearch, scrapeTHSSearch, scrapeXueqiuSearch, hashUrl, } from "../services/scraper/index.js";
const router = Router();
router.post("/", async (req, res) => {
    try {
        ...
        for (const item of uniqueNews) {
            // 宽松匹配：标题或内容包含关键词或扩展词
            const matchesQuery = item.title.includes(queryStr) || item.content.includes(queryStr);
            const matchesExpanded = expandedQueries.some((eq) => eq !== queryStr &&
                (item.title.includes(eq) || item.content.includes(eq)));
            if (!matchesQuery && !matchesExpanded)
                continue;
            try {
                const relevance = await checkRelevance(queryStr, item.title, item.content);
        ...
        filteredResults.sort((a, b) => {
            ...
            // 相关度相同时按时间排序
            return (new Date(b.news.publishedAt).getTime() -
